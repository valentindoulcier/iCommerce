	...
	if ($mysqli->connect_errno) {
		...
		if (isset($_GET['nomMarque'])) {
			...
			if (!$stmt->execute()) {
				...
				$doc = $xml->addChild('statut');
				$doc->addChild('ok', "Marque `" . $_GET['nomMarque'] . "` ajoutée");
			}
		} else {
			$doc = $xml->addChild('statut');
			$doc->addChild('error', "Paramètre `nomMarque` manquant !");
		}
	}
